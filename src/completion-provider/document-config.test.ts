...
describe("document config helpers", () => {
  ...
  it("detects import and usage snippet scopes", () => {
    ...
    expect(getSvelteSnippetScope(document as never, new Position(4, 5), "shadcn-x-button")).toBe("usage");
    expect(getSvelteSnippetScope(document as never, new Position(1, 2), "Button")).toBe("import");
    expect(getSvelteSnippetScope(document as never, new Position(5, 5), "Button")).toBe("usage");
  });
});
