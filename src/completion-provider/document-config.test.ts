...
describe("document config helpers", () => {
  ...
  it("reads the includeIndexJs setting from the workspace configuration", () => {
    ...
    vi.mocked(workspace.getConfiguration).mockReturnValue({ get });

    expect(getIncludeIndexJsSetting(createDocument(""))).toBe(false);
  ...
  it("reads and normalizes the UI alias from components.json", async () => {
    const workspaceFolder = { name: "app", uri: Uri.file("/workspace") };
    ...
    await expect(getUiAliasBaseForDocument(createDocument(""), new Set())).resolves.toBe("@/custom-ui");
  ...
  it("falls back to the default alias when components.json is missing", async () => {
    ...
    await expect(getUiAliasBaseForDocument(createDocument(""), new Set())).resolves.toBe(DEFAULT_UI_ALIAS);
    expect(window.showWarningMessage).not.toHaveBeenCalled();
  });
  ...
  it("warns once when components.json is invalid", async () => {
    ...
    await expect(getUiAliasBaseForDocument(createDocument(""), warnings)).resolves.toBe(DEFAULT_UI_ALIAS);
    await expect(getUiAliasBaseForDocument(createDocument(""), warnings)).resolves.toBe(DEFAULT_UI_ALIAS);
  ...
  it("detects import and usage snippet scopes", () => {
    ...
    expect(getSvelteSnippetScope(document, new Position(0, 3), "cni-button")).toBe("import");
    expect(getSvelteSnippetScope(document, new Position(0, 8), "shadcn-x-button")).toBe("usage");
    expect(getSvelteSnippetScope(document, new Position(1, 5), "Button")).toBe("import");
    expect(getSvelteSnippetScope(document, new Position(4, 5), "Button")).toBe("usage");
  });
});
